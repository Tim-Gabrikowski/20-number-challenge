...
class NeuralNetwork {
	constructor(layers, options) {
		...
		if (options) {
			...
			if (
				...
				console.warn(
					"Documentation Link: http://github.com/AlexDenver"
				);
		...
		for (let i = 1, j = 0; j < this.layerCount; i++, j++) {
			if (layers[i] <= 0) {
				console.error(
					"A Layer Needs To Have Atleast One Node (Neuron)."
				);
				return { layers: layers };
			}
	...
	predict(input_array) {
		if (input_array.length !== this.inputs) {
			console.error(
				`This Instance of NeuralNetwork Expects ${this.inputs} Inputs, ${input_array.length} Provided.`
			);
		...
		for (let i = 0; i < this.layerCount; i++) {
			layerResult = Matrix.multiply(
				this.layerLink[i].getWeights(),
				layerResult
			);
	...
	train(input_array, target_array) {
		...
		if (target_array.length !== this.output_nodes) {
			console.error(
				`This Instance of NeuralNetwork Expects ${this.output_nodes} Outputs, ${target_array.length} Provided.`
			);
		...
		for (let i = 0; i < this.layerCount; i++) {
			layerResult[i + 1] = Matrix.multiply(
				this.layerLink[i].getWeights(),
				layerResult[i]
			);
		...
		for (let i = this.layerCount; i > 0; i--) {
			...
			layerErrors[i - 1] = Matrix.multiply(
				Matrix.transpose(this.layerLink[i - 1].getWeights()),
				layerErrors[i]
			);
		}
	}
